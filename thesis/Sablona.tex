...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Bakalarska prace Smith normal form                                                              %%%
% Autor: Jan Plhak (408420@mail.muni.cz)                                                          %%%
% Typeset in LaTeX-2e                                                                               %%%
...
\RokOdevzdaniPrace{2015}

\AkademickyRok{2014/2015}

\Autor{Jan Plhák}{Jan Plhák}

\NazevPrace{Algoritmy pro výpočet Smithova normálního tvaru}{Algoritmy pro výpočet Smithova normálního tvaru}{Algorithms for computing the Smith normal form}
...
\VedouciPraceSTituly{Bc. Lukáš Vokřínek, PhD.} %% Neni potreba pro rigorozni prace

\StudijniProgram{Matematika}{Mathematics}

\StudijniObor{Obecná matematika}{Mathematics}

\PocetStran{ix\,$+$\,36}

\KlicovaSlova{Smithův normální tvar; Redukovaný schodovitý tvar; Algoritmus; Bachovo lemma; Čínská zbytková věta; Paralelizace}{Smith normal form; Row reduced echelon form; Algorithm; Bach's lemma; Chinese remainder theorem; Parallelization}

\Abstrakty%
{V této bakalářské práci se věnujeme algoritmům pro výpočet Smithova normálního tvaru celočíselných matic, který nalézá uplatnění například při řešení soustav celočíselných rovnic nebo při počítání simpliciálních homologií. Protože v průběhu výpočtu Smithova normálního tvaru můžeme už pro malé matice narazit na problémy s bitovou délkou mezivýsledků, uvádíme zde algoritmus, pro nějž odvozujeme odhad na mezivýsledky. Tento algoritmus modifikujeme tak, abychom mohli s využitím Čínské zbytkové věty garantovat dostatečně malou bitovou délku mezivýsledků. Modifikace algoritmu navíc umožní jistou míru paralelizace výpočtu.}%
{In this thesis we study algorithms for computing Smith normal form of integer matrices which can be used to solve system of linear Diophantine equations or in the process of computing simplicial homologies. While computing Smith normal form of even small matrices, one can easily run into problems with bit length of intermediate results. For that reason we introduce algorithm for which we present bounds on intermediate results. This algorithm is than modified using Chinese reminder theorem to provide reasonably small bounds on bit length of intermediate results. This modifications also allows certain level of parallelization.}

\TextPodekovani%
{Na tomto místě bych chtěl poděkovat Bc. Lukáši Vokřínkovi, PhD. za odborné vedení bakalářské práce a za cenné připomínky, které ovlivnily směřování celé práce.}

\TextProhlaseni%
{Prohlašuji, že jsem svoji bakalářskou práci vypracoval samostatně s~využitím informačních
zdrojů, které jsou v práci citovány.}

\DatumProhlaseni{28. května 2015}
...
\newlist{Cond}{enumerate}{10}
\setlist[Cond]{label=\text{(c\arabic*)},ref=c\arabic*}

\newlist{CondNum}{enumerate}{10}
\setlist[CondNum]{label=\text{(\arabic*)},ref=\arabic*}
...
\vloz{text_prace/05_Kapitola_04}
