...
Mějme matici $ A $ tak jako na začátku. S využitím modifikovaných algoritmů 
...
nějakým odhadem $ \omega $. Avšak všechna tato omezení byla dána na velikost,
...
zvolit $ \mu $ tak, aby platilo $ \mu > 2 \omega + 1 $.

Nechť $ \omega $ značí libovolný z našich odhadů a nechť 
$ l = 6 + \log_2(\log_2(2 \omega + 1)) $. Pak z důsledku \ref{num_prim_dus} 
vyplývá, že můžeme vybrat $ s = \ceil{\frac{2^{l}}{5 (l - 1)}} $ prvočísel 
$ p_i $ takových, že $ p_1 \cdots p_s > 2 \omega + 1 $. Zvolme 
$ \mu = p_1 \cdots p_s $. 
...
Matici $ A $ proto můžeme v průběhu výpočtu ekvivalentně reprezentovat pomocí
$ s $ matic v okruzích $ \Zbb / p_i \Zbb $ a veškeré algebraické operace 
(sčítání, odečítání, násobení) můžeme provádět paralelně přímo v těchto okruzích.

Reprezentaci matice $ A $ v průběhu výpočtu v okruhu $ \Zbb / \mu \Zbb $ budeme
...
zobrazení $ \varphi $ z důsledku \ref{Chin_Rem_Iso}.
...
redukující prvek do standardní reprezenatce, provést dělení a pak provést 
redukci v modulárním systému s výsledným koeficientem.
...
\cdots \times \Zbb / p_s \Zbb$). V obou případech je postačující vypočítat
hledané koeficienty ve standardní reprezentaci a dále pokračovat v modulárním
systému. Konkrétně se jedná o lemma \ref{Sloup_elim_GCD} - koeficient $ c $, 
...
modulární reprezentaci přepočítávat a stačí pouze pokračovat dále s vybranými
prvočísly, jejichž součin bude větší než nový odhad 
$ 2^{r - 1} \Vert \tilde{A} \Vert $. Analogicky můžeme postupovat na začátku
...
budeme potřebovat pro reprezentaci našich prvočísel při výpočtu Smithova
...
V místě, kde je potřeba provést nějaké nealgebraické operace (vypočítat koeficient),
