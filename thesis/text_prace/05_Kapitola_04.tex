...
Nehcť $ a, b \in \Zbb $. Pak existují koeficienty
$ s, t $, $ s a + t b = \gcd(a, b) $ takové, že platí $ |s| \leq |b| $ a
$ |t| \leq |a| $.
\end{lem}
...
mezivýsledky budou omezeny výrazem $ TODO $ a výsledná matice bude navíc
...
$ |t_i a_1 / s_1| \leq |a_1 a_i| $ a $ |t_k a_1 / s_1| \leq |a_1 t_k| $.
...
\begin{lem}
Buď $ T $ diagonálně dominantní matice splňující požadavky věty \ref{extra_cols}.
Pak můžeme algoritmus věty \ref{extra_cols} modifikovat tak, že všechny
mezivýsledky budou omezeny výrazem $ TODO $ a navíc transpozice výsledné matice
bude diagonálně dominantní.
\end{lem}
\begin{proof}
V každé iteraci algoritmu věty \ref{extra_cols} můžeme podle lemmatu
...
prvku $ b_{1,j} $ přiřazujeme $ t_i = t_i + tb_{i,j} $. Nakonec ještě poznamenejme
\end{proof}
