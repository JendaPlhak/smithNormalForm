...
\begin{proof}
    Uvažme následující algoritmus
        \begin{algorithm} \label{factorization_Algo}
        \Begin{
           $ X \leftarrow g $\;
    ...
    $ h > 1 $ a proměnná $ Y $ se tak s každou iterací zmenší. Protože navíc
    pro každou iteraci platí $ Y > 0 $, tak se v nejhorším případě dostaneme do
    situace, kdy $ Y = 1 $. Pak ale zřejmě $ \gcd(X, Y) = 1 $. Případně skončíme
    dříve a podmínka bude rovněž splněna.

    Dále jistě bude $ N = XY $, neboť po prvním kroku platí
    $ XY = g \frac{N}{g} = N $ a v každém dalším kroku pouze přesouváme
    prvočísla z $ Y $ do $ X $.

    Konečně neboť jsme do $ X $ po prvním kroku pouze přidávali prvočísla,
    můžeme psát $ 1 = \gcd(X,Y) = \gcd(gk,Y) $, kde $ k $ je vhodný koeficient
    takový, že $ g k = X $. To ovšem implikuje, že také $ \gcd(g,Y) = 1 $ a
    důkaz je hotov.
\end{proof}
\begin{pozn}
...
prvočíslem $ p \in \mathbb{P}$, $p \vert N $. Pro další postup budeme potřebovat
...
(\textit{ii}) $ p \vert a \Leftrightarrow p | X $.
Podobně jako v předchozím případě platí $ a + cb \equiv a + b \equiv 0 (\bmod p) $.
Z toho plyne, že $ p \vert a + b $. Avšak předpokládali jsme, že $ p \vert a $.
Proto nutně $ p \vert b $. Pak ale $ p \vert \gcd(a,b,N) $, spor.
...
\Begin{
    ...
    \For{ $k \leftarrow 1$ \KwTo $ r - 1 $} {
        $ B_k \leftarrow n \times 2 $ matice $ (col(T^{(k - 1)}, j_k) \mid col(T^{(k - 1)}, j_{k + 1})) $\;
