...
že matice $ Q_i, C_i, T_i $, které dostáváme v průběhu výpočtu, můžeme zapsat
...
Pro všechna $ i = 0, 1, \dots, r - 1 $ můžeme matice $ Q_i, C_i $ a  $ T_i $
psát jako v \ref{block_form}, kde $ R_i $ je $ (i - 1) \times (j_{i + 1} - 2) $
matice v redukovaném schodovitém tvaru. Navíc bude platit:
\begin{Cond}
    \item $ Q_i $ a $ C_i $ jsou unimodulární a
    \item prvek $ T_i[i+1, j_{i+1}] $, který budeme značit $ N_{i + 1} $, bude
    nenulový a navíc bude platit $ 0 \leq T_i[l, j_{i+1}] < N_{i+1} $ pro
